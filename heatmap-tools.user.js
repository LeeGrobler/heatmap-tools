...
(function installHeatmapWatcher() {
  console.log("Heatmap watcher booting... category-axis resolver enabled (0)");
  ...
  function buildRowResolver() {
    ...
    const categoryAxis = option.yAxis.find(a => a.type === "category");
    ...
    return function clientYToRow(clientY) {
      ...
      let result;
      try {
        result = inst.convertFromPixel({ seriesIndex: 0 }, [0, localY]);
      } catch {
        return null;
      }
      if (!result) return null;

      const rowIndex = Math.round(result[1]);

      if (rowIndex < 0 || rowIndex >= ladder.length) return null;

      return { rowIndex, price: ladder[rowIndex] };
    };
  }
  ...
  function computeRegionStats(minIndex, maxIndex) {
    ...
    const latest = data.prices.length - 1;
    const clusters = window.__liqClusters.filter(c => c.timeIndex === latest);
    const selected = clusters.filter(c => c.priceIndex >= minIndex && c.priceIndex <= maxIndex);

    return {
      clusterCount: selected.length,
      totalLiquidity: selected.reduce((s, c) => s + c.liquidity, 0)
    };
  }
  ...
  function handleSelection() {
    const resolve = ensureResolverReady();
    if (!resolve) return;

    const a = resolve(selection.startY);
    const b = resolve(selection.endY);

    if (!a || !b) return;

    const minIndex = Math.min(a.rowIndex, b.rowIndex);
    const maxIndex = Math.max(a.rowIndex, b.rowIndex);
    const minPrice = Math.min(a.price, b.price);
    const maxPrice = Math.max(a.price, b.price);
    const stats = computeRegionStats(minIndex, maxIndex);
