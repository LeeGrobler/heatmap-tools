...
This panel answers one question: "**If price moves next, which direction is more likely first?**" Not, "where will price end the day?" And not, "where should my TP definitely go?" It is a **directional context panel**.

This panel is built around four ideas:

- **`Pull Score`**: what is pulling price **right now**
...
The anchor. Everything else is relative to this.

It is mainly used to understand how far the nearest magnets are and how close price is to getting swept one way or the other.
...
- **`0.35` to `0.60`**: Moderate lean
- **`0.60` to `1.00`**: Strong lean

`0.78 (UP)`, for instance, means "very strong upward pull," and **a strong Pull Score should heavily influence one's directional bias**. It is the line that best answers, "what side price is **most likely** to **test next**?"
...
Dominance matters more for **where price may want to migrate over a larger move**, as opposed to Pull Score's next likely push.

So if `Pull Score` and `Dominance` **agree**, that usually supports **stronger conviction**, and if `Pull Score` and `Dominance` **disagree**, that usually suggests a **more complex path**.


### Nearest Magnets

These are the **closest non-zero liquidity rows** above and below the current price, represented as the **absolute price** value of the row, for example `$66,559.27`, and the **distance from price** as a percentage, for example `0.11%`.

It is useful because price often moves stepwise through liquidity.

If the **nearest magnet** agrees with `Pull Score`, it says "that side **is much more likely to be tested first**."

Nearest Magnets are mainly for **which side likely gets touched first**, and not for how far the move goes after that.
...
These are the raw totals behind `Dominance`, and since `Dominance` already summarises them, they should mostly be used to sanity-check the normalised value as needed.
